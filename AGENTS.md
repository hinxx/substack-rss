...
5. Render the static site into `index.html`.
6. Optionally publish the generated site to `gh-pages` via `publish.sh`.

## Repo Map

- `worker.py`: Main application logic. Owns feed loading, prompt generation, article summarization, JSON persistence, and static-site rendering.
